...
class MyRobot(MagicRobot):
    ...
    ex.:
    Après avoir créer dans 'components' un fichier intake_driver.py, utiliser une variable nommée "intake_beltMotor: ctre.WPI_TalonFX" déclare le type de la variable.
    Quand 'beltMotor' sera appelée depuis 'intake', ce sera un objet de type 'WPI_TalonFX'.

    Utiliser un signe égale (=) pendant la déclaration des variables tel que "intake_beltMotor = ctre.WPI_TalonFX(11)" crée l'objet.
    ...
    def initSwerve(self):
        ...
        self.frontLeftModule_driveMotor = ctre.WPI_TalonFX(26)
        self.frontLeftModule_rotateMotor = ctre.WPI_TalonFX(25)
        self.frontLeftModule_encoder = ctre.sensors.WPI_CANCoder(11)
        self.frontLeftModule_cfg = swervemodule.SwerveModuleConfig(
            nt_name="frontLeftModule", inverted=False, allow_reverse=True
        )

        self.frontRightModule_driveMotor = ctre.WPI_TalonFX(28)
        self.frontRightModule_rotateMotor = ctre.WPI_TalonFX(27)
        self.frontRightModule_encoder = ctre.sensors.WPI_CANCoder(14)
        self.frontRightModule_cfg = swervemodule.SwerveModuleConfig(
            nt_name="frontRightModule", inverted=True, allow_reverse=True
        )

        self.rearLeftModule_driveMotor = ctre.WPI_TalonFX(24)
        self.rearLeftModule_rotateMotor = ctre.WPI_TalonFX(23)
        self.rearLeftModule_encoder = ctre.sensors.WPI_CANCoder(12)
        self.rearLeftModule_cfg = swervemodule.SwerveModuleConfig(
            nt_name="rearLeftModule", inverted=True, allow_reverse=True
        )

        self.rearRightModule_driveMotor = ctre.WPI_TalonFX(21)
        self.rearRightModule_rotateMotor = ctre.WPI_TalonFX(22)
        self.rearRightModule_encoder = ctre.sensors.WPI_CANCoder(13)
